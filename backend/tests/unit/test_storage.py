...
def mock_mongo():
    ...
    with patch("app.services.storage.db") as mock_db, \
         patch("app.services.storage.fs_bucket") as mock_fs:
        
        # Setup Files Collection Mocks
        mock_db.files = MagicMock()
        mock_db.files.insert_one = AsyncMock(return_value=MagicMock(inserted_id=ObjectId()))
...
async def test_get_file_stream_found(mock_mongo):
    ...
    mock_db.files.find_one.return_value = {"_id": ObjectId(test_id), "filename": "test.csv"}
...
async def test_delete_file_success(mock_mongo):
    ...
    mock_db.files.find_one.return_value = {"_id": ObjectId(test_id), "filename": "delete_me.csv"}
